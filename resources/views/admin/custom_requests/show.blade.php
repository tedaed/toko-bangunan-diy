...
@section('content')
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-xl shadow p-6 h-fit">
            ...
            <form action="{{ route('admin.custom-requests.update-status', $customRequest->id) }}" method="POST">
                @csrf
                @method('PATCH')
                ...
                <div class="mb-4">
                    <label class="block font-semibold mb-1">
                        Catatan Status / Alasan
                    </label>

                    <textarea name="status_note" rows="3" class="w-full border rounded p-2"
                        placeholder="Contoh: Permintaan ditolak karena ukuran tidak tersedia atau bahan tidak mencukupi.">{{ old('status_note', $customRequest->status_note) }}</textarea>

                    <p class="text-sm text-gray-500 mt-1">
                        Wajib diisi jika status diubah menjadi rejected.
                    </p>
                </div>
